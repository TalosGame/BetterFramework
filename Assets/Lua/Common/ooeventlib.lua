...
function _M:Connect(handler,target)
    ...
    table.insert(self.handlers, {h = handler, t = target})
    local t = { }
...
function _M:Disconnect(handler,target)
    assert(self ~= nil and type(self) == "table", "Invalid Event (make sure you're using ':' not '.')")
    ...
    if not handler then
        ...
        for k, v in pairs(self.handlers) do
            if v.h == handler and v.t == target then
                self.handlers[k] = nil
                return k
            end
        end
    ...
    for k, v in pairs(self.handlers) do
        ...
        spawn(function() 
            if(v.t ~= nil) then
                tar = v.t;
                hand = v.h;
                hand(tar, unpack(self.args))
            else
                v.h(unpack(self.args))
            end
            i = i - 1
